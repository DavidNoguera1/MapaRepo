...
class User {
  ...
  static async findByName(name, limit = 10, offset = 0) {
    const query = `
      SELECT id, user_name, email, phone, cedula, profile_picture_url, role, is_active, created_at, updated_at
      FROM users
  ...
  static async findByFilters(filters = {}, limit = 10, offset = 0, sortOrder = 'desc') {
    let query = `
      SELECT id, user_name, email, phone, cedula, profile_picture_url, role, is_active, created_at, updated_at
      FROM users
      WHERE 1=1
    ...
    query += ` ORDER BY created_at ${order} LIMIT $${paramIndex} OFFSET $${paramIndex + 1}`;
